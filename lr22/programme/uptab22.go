...
func Uptab22( user *user.User ) {
	if user.X < user.Gfb + 1 {
		...
		user.St = function.Abrunden_double( 0, ( user.Rw + 938.24 ) ); // abrunden auf ganze euro
	} else if user.X < 274613 {
		user.St = function.Abrunden_double( 0,( user.X * 0.42 ) - 9267.53 ); // abrunden auf ganze euro
	} else {
		user.St = function.Abrunden_double( 0, ( user.X * 0.45 ) - 17602.28 ); // abrunden auf ganze euro
	}
