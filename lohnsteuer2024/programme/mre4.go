...
func Mre4(user *user.User) {
	...
	if user.Zvbezj == 0 {
		...
		if user.Lzz == 1 {
			...
			user.Hfvb = (function.Tab(2, user.J) / 12) * float64(user.Zmvb)
			user.Fvbz = function.Aufrunden_double(0, ((function.Tab(3, user.J) / 12) * float64(user.Zmvb)))
		} else {
		...
		user.Fvb = function.Aufrunden_double(2, (user.Vbezb*function.Tab(1, user.J))*0.01)
		...
		user.Fvbso = function.Aufrunden_double(2, (user.Fvb + (user.Vbezbso*function.Tab(1, user.J))*0.01))
		...
		user.Fvbzso = function.Aufrunden_double(0, (user.Fvbz + user.Vbezbso*0.01))

		if user.Fvbzso > user.Hfvbzso {
			user.Fvbzso = function.Aufrunden_double(0, user.Hfvbzso)
		}
		...
		if user.Fvbz > user.Hfvbz {
			user.Fvbz = function.Aufrunden_double(0, user.Hfvbz)
		}
		Mre4alte(user)
	}
