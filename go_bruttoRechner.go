...
var file_name, PGM, opt_lr, opt_format string;
var file_datei *os.File;
var spooldir = "/tmp/mwegner";
var pdf *gofpdf.Fpdf;
...
func process_arguments( args []string ) int {
	...
	if ( opt_manuell == 1 && opt_auto == 1 ) {
		...
		fmt.Println( "\n\nDAS IST EINE VIEW ANSICHT !!! (da kein df angegeben wurde)\nMit ctrl + c zum Beenden des Programms \n\n" );
	...
	if ( opt_auto == 1 ) {
		...
		for i := 0 ; i < len(users); i++ {
			...
			if opt_lr == "lr19" {
				...
				fmt.Println( "es ist ein LR angegeben Programm wurde Beendet" );
				return(1);
			}

			if user.R == 1 {
				kst19.Kst_19_MW( &user );
			}
			sv19.Sv_19_MW( &user );
			sum19.Sum_19_MW( &user );
		...
		// code for lr sv sum kst
		if opt_lr == "lr19" {
			lr19.Lr_19_MW( &user_daten );
		} else if opt_lr == "lr20" {
			lr20.Lr_20_MW( &user_daten );
		} else if opt_lr == "lr21" {
			lr21.Lr_21_MW( &user_daten );
		} else if opt_lr == "lr22" {
			lr22.Lr_22_MW( &user_daten );
		}

		if user_daten.R == 1 {
			kst19.Kst_19_MW( &user_daten );
		}
		sv19.Sv_19_MW( &user_daten );
		sum19.Sum_19_MW( &user_daten );
		...
		if os.IsNotExist(err) {
			if ( opt_format == "text" || opt_format == "csv" ) {
				file_datei, err = os.Create( file_path );
			}
